...
DB_NAME="${CF_D1_NAME:-nangongyuan-blog-db}"
R2_NAME="${CF_R2_NAME:-nangongyuan-blog-images}"
KV_NAME="${CF_KV_NAME:-nangongyuan-blog-cache}"
SITE_URL="${SITE_URL:-${NEXT_PUBLIC_SITE_URL:-https://blog.ngy123.com}}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: npm run cf:init -- [--with-kv] [--site-url=https://blog.ngy123.com] [--db-name=nangongyuan-blog-db] [--r2-name=nangongyuan-blog-images] [--kv-name=nangongyuan-blog-cache]" >&2
      exit 1
      ;;
  esac
done
