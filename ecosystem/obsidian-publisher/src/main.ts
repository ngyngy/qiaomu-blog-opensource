...
export default class QmblogPublisher extends Plugin {
  ...
  async onload() {
    ...
    this.addRibbonIcon("upload-cloud", "发布到 南宫远的博客", async () => {
      await this.publishCurrentNote();
    });

    // 2. Command palette
    this.addCommand({
      id: "publish-to-nangongyuan-blog",
      name: "发布到 南宫远的博客",
      editorCallback: () => {
    ...
    this.statusBarEl.setText("南宫远的博客");
  ...
  private setStatus(text: string, revertMs?: number) {
    ...
    if (revertMs) {
      setTimeout(() => {
        if (this.statusBarEl) this.statusBarEl.setText("南宫远的博客");
      }, revertMs);
    }
  }
  ...
  private openPublishModal(file: TFile, content: string, title: string) {
    const modal = new PublishModal(
      ...
      async (options, onProgress) => {
        this.setStatus("南宫远的博客 \u23F3");
        try {
          const result = await this.doPublish(file, content, options, onProgress);
          if (result.success) {
            this.setStatus("南宫远的博客 \u2713", 3000);
          } else {
            this.setStatus("南宫远的博客 \u2717", 5000);
          }
          return result;
        } catch (e) {
          this.setStatus("南宫远的博客 \u2717", 5000);
